...
void DX11Renderer::BindMainWindowRenderTarget()
{
    ...
    m_context->ClearRenderTargetView(m_mainRenderTargetView.Get(), clearColor);
}
...
void DX11Renderer::CreateRenderTarget()
{
    ...
    hr = m_device->CreateRenderTargetView(pBackBuffer.Get(), nullptr, m_mainRenderTargetView.GetAddressOf());
    if(FAILED(hr))
    {
        return;
    }
